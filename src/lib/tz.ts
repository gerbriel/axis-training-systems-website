/**
 * Timezone primitives for the booking system (browser copy of _shared/tz.ts).
 *
...
/**
 * Absolute UTC instant for "date D at wall-clock time T in zone Z".
 *
 * Returns null only when the wall time does not exist (spring-forward gap) and
 * `nonexistent: 'reject'` is set.
 *
 * The offset is a function OF the instant we are looking for, so it cannot be
 * inverted directly: read the wall fields as if they were UTC, probe the zone's
 * offset a day before / at / a day after that pretend instant (near a transition
 * these disagree, giving up to 3 candidates), then keep only the candidates that
 * render back to the requested wall time. 0 valid -> nonexistent, 1 -> normal,
 * 2 -> ambiguous.
 */
export function zonedTimeToUtc(
  ...
  // Applying the PRE-transition offset lands past the gap.
...
/** Parses a Postgres TIME ('HH:MM' or 'HH:MM:SS'). */
 ...
 * This is exactly what coach_schedules.start_time / coach_availability_blocks.start_time
 * mean once we say they are authored in the coach's zone.
 */
...
/** Minutes since midnight -> 'HH:MM'. Inverse of timeToMinutes for values < 1440. */
