 ...
 *
 * The meets and testimonials the public site shows have always lived in code
 * (STATIC_MEETS here, and each coach's testimonials in coaches.ts). That is why
 * the admin panels — which read the database — could not see or edit them. This
 * copies them into the database ONCE, using the very same code data the site
 * renders, so there is no hand-transcribed second copy to drift.
 *
 ...
 * coach+quote) and skips anything already there, so running it twice imports
 * nothing the second time rather than duplicating the site.
 *
 * Blog posts are deliberately NOT imported: a database post is addressed by its
 * row id, so importing the built-in posts would change their /blog/<slug> URLs
 * from the human slugs they have now to UUIDs. That needs a slug column first —
 * a separate change — so the founding posts stay in code until then.
 */
