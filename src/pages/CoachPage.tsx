  ...
  return (
    <div style={{ background: '#000000', minHeight: '100vh' }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(8px)', borderBottom: '1px solid #222222', padding: '0 2rem', display: 'flex', alignItems: 'center', height: '3.5rem', gap: '1.5rem' }}>
        ...
        <span style={{ color: '#888888', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.2em', textTransform: 'uppercase' }}>Coaches</span>
        <span style={{ color: '#888888' }}>›</span>
        <span style={{ color: '#c7c7c7', fontSize: '.7rem', fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' }}>{coach.name}</span>

        <div style={{ marginLeft: 'auto', display: 'flex', gap: '1rem', alignItems: 'center' }}>
          <a
            href={href('/')}
            style={{ color: '#c7c7c7', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', textDecoration: 'none', transition: 'color .15s' }}
            ...
            <div style={{ flexShrink: 0 }}>
              {coach.photo ? (
                <img
                  ...
                  style={{ width: 140, height: 140, borderRadius: '50%', objectFit: 'cover', border: '3px solid #222222' }}
                />
              ) : (
                <div style={{ width: 120, height: 120, borderRadius: '50%', background: '#0d0d0d', border: '2px solid #222222', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ color: '#fff', fontWeight: 900, fontSize: '3rem' }}>{coach.firstName[0]}</span>
                </div>
              )}
            </div>
            {/* Name + role */}
            <div style={{ flex: 1, minWidth: 260 }}>
              <p style={{ color: '#fff', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '.5rem' }}>{coach.role}</p>
              ...
              <p style={{ color: '#c7c7c7', fontSize: '1rem', lineHeight: 1.6, maxWidth: 520, marginBottom: '2rem' }}>{coach.tagline}</p>
      ...
      {/* ── Stats bar ────────────────────────────────────────────────────── */}
      <section style={{ borderBottom: '1px solid #0d0d0d', background: '#000000' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 2rem', display: 'flex', flexWrap: 'wrap' }}>
          {coach.stats.map((s, i) => (
            <div key={i} style={{ flex: '1 1 120px', padding: '2rem 1.5rem', borderRight: i < coach.stats.length - 1 ? '1px solid #0d0d0d' : 'none' }}>
              <p style={{ color: '#fff', fontWeight: 900, fontSize: '1.6rem', letterSpacing: '-.02em', marginBottom: '.25rem' }}>{s.value}</p>
              <p style={{ color: '#888888', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase' }}>{s.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ── Bio ──────────────────────────────────────────────────────────── */}
      <section style={{ padding: '5rem 2rem', borderBottom: '1px solid #0d0d0d' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', display: 'grid', gap: '3rem', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
          <div>
            <p style={{ color: '#fff', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '1.5rem' }}>About</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              {coach.bio.map((p, i) => (
                <p key={i} style={{ color: '#888888', fontSize: '.95rem', lineHeight: 1.8 }}>{p}</p>
              ))}
            </div>
          </div>
          <div>
            {/* Philosophy pull-quote */}
            <p style={{ color: '#fff', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '1.5rem' }}>Coaching Philosophy</p>
            ...
            {/* Specialties */}
            <p style={{ color: '#fff', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '1rem' }}>Specialties</p>
            <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: '.6rem' }}>
              {coach.specialties.map((s, i) => (
                <li key={i} style={{ display: 'flex', alignItems: 'center', gap: '.75rem', color: '#c7c7c7', fontSize: '.875rem' }}>
      ...
      {/* ── Services ─────────────────────────────────────────────────────── */}
      <section style={{ padding: '5rem 2rem', background: '#000000', borderBottom: '1px solid #0d0d0d' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <p style={{ color: '#fff', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '.75rem' }}>Services</p>
          ...
          <div style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
            {coach.services.map((s, i) => (
              <div key={i} style={{ background: '#000000', border: '1px solid #222222', padding: '2rem', borderRadius: '.25rem', display: 'flex', flexDirection: 'column', gap: '.75rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
                  <p style={{ color: '#fff', fontWeight: 900, fontSize: '.9rem', textTransform: 'uppercase', letterSpacing: '.05em' }}>{s.name}</p>
                  <span style={{ color: '#fff', fontWeight: 900, fontSize: '.875rem', whiteSpace: 'nowrap' }}>{s.price}</span>
                </div>
                <p style={{ color: '#c7c7c7', fontSize: '.85rem', lineHeight: 1.7 }}>{s.description}</p>
                <a
                  href={applyHref(coach.slug)}
                  style={{ marginTop: 'auto', display: 'inline-block', background: 'transparent', border: '1px solid #222222', color: '#888888', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', padding: '.6rem 1.25rem', borderRadius: '.2rem', textDecoration: 'none', transition: 'border-color .15s, color .15s', alignSelf: 'flex-start' }}
      ...
      <section style={{ padding: '5rem 2rem', borderBottom: '1px solid #0d0d0d' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <p style={{ color: '#fff', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '.75rem' }}>Results</p>
          <h2 style={{ color: '#fff', fontWeight: 900, fontSize: 'clamp(1.75rem, 4vw, 3rem)', textTransform: 'uppercase', letterSpacing: '-.02em', marginBottom: '3rem' }}>
            Athlete Outcomes
          </h2>
          <div style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))' }}>
            {coach.testimonials.map((t, i) => (
              <div key={i} style={{ background: '#000000', border: '1px solid #222222', padding: '2rem', borderRadius: '.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {/* Quote mark */}
                <span style={{ color: '#fff', fontSize: '2rem', fontWeight: 900, lineHeight: 1, opacity: .6 }}>"</span>
                <p style={{ color: '#aaa', fontSize: '.875rem', lineHeight: 1.8, flex: 1 }}>{t.quote}</p>
                <div style={{ borderTop: '1px solid #0d0d0d', paddingTop: '1rem', display: 'flex', alignItems: 'center', gap: '.875rem' }}>
                  {t.photo ? (
                    <img
                      src={t.photo}
                      alt={t.athlete}
                      style={{ width: 42, height: 42, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '2px solid #222222' }}
                    />
                  ) : (
                    <div style={{ width: 42, height: 42, borderRadius: '50%', background: '#0d0d0d', border: '2px solid #222222', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ color: '#fff', fontWeight: 900, fontSize: '1rem' }}>{t.athlete[0]}</span>
                    </div>
                  )}
                  <div>
                    <p style={{ color: '#fff', fontWeight: 700, fontSize: '.8rem', marginBottom: '.2rem' }}>{t.athlete}</p>
                    <p style={{ color: '#fff', fontSize: '.6rem', fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' }}>{t.result}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section style={{ position: 'relative', padding: '7rem 2rem', overflow: 'hidden' }}>
        ...
        {/* Dark overlay */}
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(0,0,0,0.55) 0%, rgba(0,0,0,0.75) 60%, rgba(0,0,0,0.95) 100%)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative', maxWidth: 700, margin: '0 auto', textAlign: 'center' }}>
          <p style={{ color: '#fff', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '1rem' }}>Ready to Start?</p>
      ...
      <div style={{ background: '#000000', borderTop: '1px solid #0d0d0d', padding: '1.5rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
          <a href={href('/')} style={{ color: '#888888', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', textDecoration: 'none' }}>← All Coaches</a>
          <a href={adminHref(coach.slug)} style={{ color: '#888888', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', textDecoration: 'none' }}>Coach Admin</a>
        </div>
      </div>
    </div>
  )
