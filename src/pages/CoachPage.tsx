  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: 'var(--nav-overlay)', backdropFilter: 'blur(8px)', borderBottom: '1px solid var(--border)', padding: '0 2rem', display: 'flex', alignItems: 'center', height: '3.5rem', gap: '1.5rem' }}>
        ...
        <span style={{ color: 'var(--text-3)', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.2em', textTransform: 'uppercase' }}>Coaches</span>
        <span style={{ color: 'var(--text-3)' }}>›</span>
        <span style={{ color: 'var(--text-2)', fontSize: '.7rem', fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' }}>{coach.name}</span>

        <div style={{ marginLeft: 'auto', display: 'flex', gap: '1rem', alignItems: 'center' }}>
          <a
            href={href('/')}
            style={{ color: 'var(--text-2)', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', textDecoration: 'none', transition: 'color .15s' }}
        ...
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3rem', alignItems: 'flex-start' }}>
            ...
            <div style={{ flexShrink: 0 }}>
              {coach.photo ? (
                <img
                  ...
                  style={{ width: 140, height: 140, borderRadius: '50%', objectFit: 'cover', border: '3px solid var(--border)' }}
                />
              ) : (
            ...
            <div style={{ flex: 1, minWidth: 260 }}>
              <p style={{ color: 'var(--text)', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '.5rem' }}>{coach.role}</p>
              <h1 style={{ color: 'var(--text)', fontWeight: 900, fontSize: 'clamp(2.5rem, 7vw, 5rem)', textTransform: 'uppercase', letterSpacing: '-.02em', lineHeight: 0.9, marginBottom: '1.25rem' }}>
                {coach.name}
              </h1>
              <p style={{ color: 'var(--text-2)', fontSize: '1rem', lineHeight: 1.6, maxWidth: 520, marginBottom: '2rem' }}>{coach.tagline}</p>
      ...
      <section style={{ borderBottom: '1px solid var(--surface)', background: 'var(--bg)' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 2rem', display: 'flex', flexWrap: 'wrap' }}>
          {coach.stats.map((s, i) => (
            <div key={i} style={{ flex: '1 1 120px', padding: '2rem 1.5rem', borderRight: i < coach.stats.length - 1 ? '1px solid var(--surface)' : 'none' }}>
              <p style={{ color: 'var(--text)', fontWeight: 900, fontSize: '1.6rem', letterSpacing: '-.02em', marginBottom: '.25rem' }}>{s.value}</p>
              <p style={{ color: 'var(--text-3)', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase' }}>{s.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '5rem 2rem', borderBottom: '1px solid var(--surface)' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', display: 'grid', gap: '3rem', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
          <div>
            <p style={{ color: 'var(--text)', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '1.5rem' }}>About</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              {coach.bio.map((p, i) => (
                <p key={i} style={{ color: 'var(--text-3)', fontSize: '.95rem', lineHeight: 1.8 }}>{p}</p>
              ))}
            </div>
          </div>
          <div>
            ...
            {/* Specialties */}
            <p style={{ color: 'var(--text)', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '1rem' }}>Specialties</p>
      ...
      <section style={{ padding: '5rem 2rem', background: 'var(--bg)', borderBottom: '1px solid var(--surface)' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          <h2 style={{ color: 'var(--text)', fontWeight: 900, fontSize: 'clamp(1.75rem, 4vw, 3rem)', textTransform: 'uppercase', letterSpacing: '-.02em', marginBottom: '3rem' }}>
            Work With {coach.firstName}
          </h2>
          <div style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
            {coach.services.map((s, i) => (
              <div key={i} style={{ background: 'var(--bg)', border: '1px solid var(--border)', padding: '2rem', borderRadius: '.25rem', display: 'flex', flexDirection: 'column', gap: '.75rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
                  <p style={{ color: 'var(--text)', fontWeight: 900, fontSize: '.9rem', textTransform: 'uppercase', letterSpacing: '.05em' }}>{s.name}</p>
                  <span style={{ color: 'var(--text)', fontWeight: 900, fontSize: '.875rem', whiteSpace: 'nowrap' }}>{s.price}</span>
                </div>
                <p style={{ color: 'var(--text-2)', fontSize: '.85rem', lineHeight: 1.7 }}>{s.description}</p>
                <a
                  href={applyHref(coach.slug)}
          ...
          <div style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))' }}>
            {coach.testimonials.map((t, i) => (
              <div key={i} style={{ background: 'var(--bg)', border: '1px solid var(--border)', padding: '2rem', borderRadius: '.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div style={{ borderTop: '1px solid var(--surface)', paddingTop: '1rem', display: 'flex', alignItems: 'center', gap: '.875rem' }}>
                  {t.photo ? (
                    <img
                      src={t.photo}
                      alt={t.athlete}
                      style={{ width: 42, height: 42, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '2px solid var(--border)' }}
                    />
                  ) : (
                    <div style={{ width: 42, height: 42, borderRadius: '50%', background: 'var(--surface)', border: '2px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ color: 'var(--text)', fontWeight: 900, fontSize: '1rem' }}>{t.athlete[0]}</span>
                    </div>
                  )}
                  <div>
                    <p style={{ color: 'var(--text)', fontWeight: 700, fontSize: '.8rem', marginBottom: '.2rem' }}>{t.athlete}</p>
                    <p style={{ color: 'var(--text)', fontSize: '.6rem', fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' }}>{t.result}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div style={{ background: 'var(--bg)', borderTop: '1px solid var(--surface)', padding: '1.5rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
          <a href={href('/')} style={{ color: 'var(--text-3)', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', textDecoration: 'none' }}>← All Coaches</a>
          <a href={adminHref(coach.slug)} style={{ color: 'var(--text-3)', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', textDecoration: 'none' }}>Coach Admin</a>
        </div>
      </div>
    </div>
  )
