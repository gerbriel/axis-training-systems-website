  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: 'var(--nav-overlay)', backdropFilter: 'blur(8px)', borderBottom: '1px solid var(--border)', padding: '0 2rem', display: 'flex', alignItems: 'center', height: '3.5rem', gap: '1.5rem' }}>
        ...
        <span style={{ color: 'var(--text-2)', fontSize: '.7rem', fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' }}>Blog</span>
      </nav>

      {/* Header */}
      <section style={{ padding: '6rem 2rem 4rem', borderBottom: '1px solid var(--surface)' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <p style={{ color: 'var(--text)', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '.75rem' }}>Axis Blog</p>
          ...
          <p style={{ color: 'var(--text-2)', fontSize: '.9rem', marginTop: '1.25rem', maxWidth: 480, lineHeight: 1.7 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', background: 'var(--surface)' }}>
          {allPosts.map(post => (
            <a
              ...
              style={{ display: 'block', background: 'var(--bg)', padding: '2.5rem 2rem', textDecoration: 'none', transition: 'background .15s' }}
            ...
            >
              ...
              <p style={{ color: 'var(--text-2)', fontSize: '.875rem', lineHeight: 1.7, marginBottom: '1.25rem', maxWidth: 600 }}>{post.summary}</p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem' }}>
                  {post.coachSlug ? (
                    <a
                      ...
                      style={{ color: 'var(--text)', fontSize: '.7rem', fontWeight: 700, textDecoration: 'none' }}
                    ...
                    <span style={{ color: 'var(--text-3)', fontSize: '.7rem', fontWeight: 700 }}>{post.author}</span>
                  )}
                  <span style={{ color: 'var(--text-3)' }}>·</span>
                  <span style={{ color: 'var(--text-3)', fontSize: '.7rem' }}>{post.date}</span>
                </div>
                <span style={{ color: 'var(--text)', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.15em', textTransform: 'uppercase' }}>Read →</span>
              </div>
            </a>
          ))}
        </div>
      ...
      {/* Footer strip */}
      <div style={{ background: 'var(--bg)', borderTop: '1px solid var(--surface)', padding: '1.5rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <a href={href('/')}>
          <img src={`${BASE}logo.svg`} alt="Axis" style={{ height: 20, filter: 'var(--logo-filter)' }} />
        </a>
        <a href={href('/')} style={{ color: 'var(--text-3)', fontSize: '.65rem', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', textDecoration: 'none' }}>← Back to site</a>
      </div>
    </div>
  )
}
