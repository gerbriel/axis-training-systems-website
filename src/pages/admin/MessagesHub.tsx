 ...
 * the conversation with someone who already wrote in; the newsletter is the
 * list of people waiting to hear from you. They were two rail entries and one
 * sentence, so they are one entry and two sub-tabs now.
 *
 * The unread badge stays on the rail's Messages item and still counts inbox
 * conversations only. Nothing about it changed.
 */

type Sub = 'inbox' | 'newsletter'
...
export default function MessagesHub({ isDemo = false }: { isDemo?: boolean }) {
  ...
  return (
    <div>
      ...
      {sub === 'newsletter' && <NewsletterPanel isDemo={isDemo} />}
    </div>
  )
}
