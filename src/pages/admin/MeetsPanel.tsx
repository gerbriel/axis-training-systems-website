    ...
    <div style={{ padding:'2rem', maxWidth:960 }}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1.75rem',flexWrap:'wrap',gap:'1rem'}}>
          <div>
            <h2 style={{color:'var(--text)',fontSize:'1.25rem',fontWeight:900,marginBottom:'.25rem'}}>
              Meet Listings {pendingCount>0 && <span style={{background:'#c8102e',color:'var(--text)',fontSize:'.6rem',fontWeight:900,borderRadius:'10rem',padding:'.15rem .55rem',marginLeft:'.5rem',verticalAlign:'middle'}}>{pendingCount}</span>}
            </h2>
            <p style={{color:'var(--steel)',fontSize:'.8rem'}}>Manage upcoming powerlifting meets and events.</p>
          </div>
          <div style={{display:'flex',gap:'.5rem'}}>
            ...
            <button onClick={openCreate} style={{background:'#0d5bae',border:'1px solid #0d5bae',color:'var(--text)',fontSize:'.65rem',fontWeight:900,letterSpacing:'.12em',textTransform:'uppercase',padding:'.5rem 1rem',borderRadius:'.2rem',cursor:'pointer',fontFamily:'inherit'}}
      ...
      {(mode==='create'||mode==='edit') && (
        <div>
          <div style={{display:'flex',alignItems:'center',gap:'1rem',marginBottom:'1.75rem',flexWrap:'wrap'}}>
            ...
            <h2 style={{color:'var(--text)',fontWeight:900,fontSize:'1.1rem',textTransform:'uppercase'}}>{mode==='create'?'New Meet / Event':'Edit Meet'}</h2>
          ...
          <div style={{display:'flex',gap:'.75rem',marginTop:'1.5rem'}}>
            <button onClick={mode==='create'?saveCreate:saveEdit} disabled={saving||!meetValid} style={{background:'#0d5bae',border:'none',color:'var(--text)',fontWeight:900,fontSize:'.75rem',letterSpacing:'.15em',textTransform:'uppercase',padding:'.75rem 1.75rem',borderRadius:'.25rem',cursor:'pointer',fontFamily:'inherit',opacity:saving||!meetValid?0.45:1}}>
      ...
      {mode==='list' && (
        <>
          ...
          {loading ? (
            <div style={{padding:'3rem',textAlign:'center',color:'var(--border)',fontSize:'.875rem'}}>Loading…</div>
          ) : filtered.length===0 ? (
            <div style={{background:'var(--bg)',border:'1px solid var(--surface)',borderRadius:'.25rem',padding:'3rem 2rem',textAlign:'center'}}><p style={{color:'var(--border)',fontSize:'.875rem'}}>{filterStatus==='pending'?'No pending meet submissions.':'No meets listed.'}</p></div>
          ) : (
                    ...
                    <div style={{display:'flex',gap:'1rem',alignItems:'flex-start',padding:'1.25rem 1.5rem',cursor:'pointer',flexWrap:'wrap'}} onClick={()=>setExpandedId(isExpanded?null:item.id)}>
                      <div style={{flex:1,minWidth:200}}>
                        <div style={{display:'flex',gap:'.5rem',alignItems:'center',marginBottom:'.35rem',flexWrap:'wrap'}}>
                          ...
                          {item.meetType && <span style={{color:'var(--text-dim)',fontSize:'.65rem',fontWeight:600}}>{item.meetType}</span>}
                          {item.federation && <span style={{color:'var(--text-dim)',fontSize:'.65rem',fontWeight:600}}>{item.federation}</span>}
                        </div>
                        <p style={{color:'var(--text)',fontWeight:700,fontSize:'.925rem'}}>{item.meetName}</p>
                        <p style={{color:'var(--text-4)',fontSize:'.8rem',marginTop:'.25rem'}}>{item.meetDate}{item.meetLocation?' · '+item.meetLocation:''}</p>
                        <p style={{color:'var(--border-mid)',fontSize:'.65rem',marginTop:'.4rem'}}>{new Date(item.submittedAt).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}{item.reviewedAt&&(' · Listed ' + new Date(item.reviewedAt).toLocaleDateString('en-US',{month:'short',day:'numeric'}))}</p>
                        {item.status==='rejected'&&item.rejectionNote&&<p style={{color:'#c8102e',fontSize:'.75rem',marginTop:'.4rem'}}>Note: {item.rejectionNote}</p>}
                      </div>
                      <span style={{color:'var(--border-mid)',fontSize:'.7rem',flexShrink:0,paddingTop:'.2rem'}}>{isExpanded?'▲':'▼'}</span>
                    </div>

                    {isExpanded && (
                      <div style={{borderTop:'1px solid var(--surface)',padding:'1.5rem',display:'flex',flexDirection:'column',gap:'1.25rem'}}>
                        {item.meetNote && <div><p style={lbl}>Note</p><p style={{color:'var(--text-3)',fontSize:'.85rem',lineHeight:1.6}}>{item.meetNote}</p></div>}
