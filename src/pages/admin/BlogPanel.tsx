            ...
            <div key={sec._id} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '.2rem', padding: '.75rem' }}>
              <div style={{ display: 'flex', gap: '.5rem', alignItems: 'center', marginBottom: sec.type === 'divider' ? 0 : '.5rem', flexWrap: 'wrap' }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: '.3rem' }}>
                  ...
                  <button onClick={() => del(sec._id)} style={{ background:'none',border:'1px solid var(--border)',color:'var(--text-4)',fontSize:'.65rem',padding:'.2rem .5rem',borderRadius:'.15rem',cursor:'pointer',fontFamily:'inherit' }}>×</button>
                </div>
              </div>
    ...
    <div style={{ padding:'2rem', maxWidth:960 }}>
      ...
      {(mode==='create'||mode==='edit') && (
        <div>
          <div style={{display:'flex',alignItems:'center',gap:'1rem',marginBottom:'1.75rem',flexWrap:'wrap'}}>
            ...
            <h2 style={{color:'var(--text)',fontWeight:900,fontSize:'1.1rem',textTransform:'uppercase'}}>{mode==='create'?'New Blog Post':'Edit Blog Post'}</h2>
          ...
          <div style={{display:'flex',gap:'.75rem',marginTop:'1.5rem'}}>
            <button onClick={mode==='create'?saveCreate:saveEdit} disabled={saving||!blogValid} style={{background:'#c8102e',border:'none',color:'var(--text)',fontWeight:900,fontSize:'.75rem',letterSpacing:'.15em',textTransform:'uppercase',padding:'.75rem 1.75rem',borderRadius:'.25rem',cursor:'pointer',fontFamily:'inherit',opacity:saving||!blogValid?0.45:1}}>
      ...
      {mode==='list' && (
        <>
          ...
          {!isDemo && !loading && items.length===0 && <ImportSiteContent kind="blog" onImported={refresh} />}
          ...
          {loading ? (
            ...
            <div style={{background:'var(--bg)',border:'1px solid var(--surface)',borderRadius:'.25rem',padding:'3rem 2rem',textAlign:'center'}}><p style={{color:'var(--border)',fontSize:'.875rem'}}>{filterStatus==='pending'?'No pending blog posts.':'No reviewed posts.'}</p></div>
          ) : (
                    ...
                    <div style={{display:'flex',gap:'1rem',alignItems:'flex-start',padding:'1.25rem 1.5rem',cursor:'pointer',flexWrap:'wrap'}} onClick={()=>setExpandedId(isExpanded?null:item.id)}>
                      <div style={{flex:1,minWidth:200}}>
                        <div style={{display:'flex',gap:'.5rem',alignItems:'center',marginBottom:'.35rem',flexWrap:'wrap'}}>
                          ...
                          <span style={{color:'var(--text-dim)',fontSize:'.65rem',fontWeight:600}}>by {item.coachName}</span>
                        </div>
                        <p style={{color:'var(--text)',fontWeight:700,fontSize:'.925rem'}}>{item.title}</p>
                        {item.summary && <p style={{color:'var(--steel)',fontSize:'.8rem',marginTop:'.35rem',lineHeight:1.5}}>{item.summary}</p>}
                        <p style={{color:'var(--border-mid)',fontSize:'.65rem',marginTop:'.4rem'}}>{new Date(item.submittedAt).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}{item.reviewedAt&&(' · Reviewed ' + new Date(item.reviewedAt).toLocaleDateString('en-US',{month:'short',day:'numeric'}))}</p>
