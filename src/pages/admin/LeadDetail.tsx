...
function Row({ label, value }: { label: string; value?: string | null }) {
  ...
  return (
    <div style={{ display: 'flex', gap: '1rem', padding: '.6rem 0', borderBottom: '1px solid #111', fontSize: '.85rem' }}>
      <span style={{ minWidth: '10rem', color: '#444', flexShrink: 0 }}>{label}</span>
      <span style={{ color: '#ccc', wordBreak: 'break-word' }}>{value}</span>
    </div>
  )
}
...
function SectionHead({ label }: { label: string }) {
  return (
    <p style={{ color: '#e63e3e', fontSize: '.6rem', fontWeight: 900, letterSpacing: '.3em', textTransform: 'uppercase', margin: '1.75rem 0 .5rem' }}>{label}</p>
  )
}
...
export default function LeadDetail({ lead, onClose, onUpdate, isDemo = false }: Props) {
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(4px)', padding: '2rem 1rem' }}
      onClick={e => { if (e.target === e.currentTarget) onClose() }}
    >
      <div className="w-full relative" style={{ maxWidth: 680, background: '#0a0a0a', border: '1px solid #1e1e1e', borderRadius: '.25rem', padding: '2.5rem' }}>
        {/* Close */}
        ...
        {/* Admin controls */}
        <div style={{ marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px solid #1a1a1a', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <p style={{ color: '#e63e3e', fontSize: '.6rem', fontWeight: 900, letterSpacing: '.3em', textTransform: 'uppercase' }}>Admin</p>
          ...
          <button
            ...
            style={{ background: saving ? '#7a1f1f' : '#e63e3e', border: 'none', color: '#fff', fontWeight: 900, fontSize: '.75rem', letterSpacing: '.15em', textTransform: 'uppercase', padding: '.875rem', borderRadius: '.25rem', cursor: 'pointer' }}
            onMouseEnter={e => { if (!saving) e.currentTarget.style.background = '#c42e2e' }}
            onMouseLeave={e => { if (!saving) e.currentTarget.style.background = '#e63e3e' }}
          >
            {saving ? 'Saving…' : 'Save Changes'}
          </button>
          {isDemo && (
            <p style={{ color: '#5c3d00', fontSize: '.7rem', textAlign: 'center' }}>
