...
export default function Philosophy() {
  return (
    <section id="philosophy" style={{ background: 'var(--bg)', borderTop: '1px solid var(--surface)', borderBottom: '1px solid var(--surface)', padding: '8rem 1.5rem' }}>
      <div className="max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          {/* Left */}
          <div>
            <p className="label-rule" style={{ color: 'var(--text)', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '1.5rem' }}>
              Who We Are
            </p>
            <h2 style={{ fontWeight: 900, textTransform: 'uppercase', letterSpacing: '-.03em', lineHeight: 0.9, fontSize: 'clamp(2rem,5vw,4rem)', color: 'var(--text)', marginBottom: '2rem' }}>
              Coaching you can trust.
            </h2>
            <p style={{ color: 'var(--text-2)', fontSize: '.9rem', lineHeight: 1.8, marginBottom: '1.25rem' }}>
            ...
            <p style={{ color: 'var(--text-2)', fontSize: '.9rem', lineHeight: 1.8, marginBottom: '2rem' }}>
            ...
            <blockquote style={{ borderLeft: '3px solid #f5b935', paddingLeft: '1.25rem' }}>
              ...
              <p style={{ color: 'var(--text)', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.2em', textTransform: 'uppercase' }}>— Ronnie Vallejo, Founder</p>
            </blockquote>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ position: 'relative', overflow: 'hidden', borderRadius: '.15rem' }}>
              ...
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, rgba(0,0,0,0.65) 0%, transparent 50%)' }} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              {PILLARS.map(p => (
                <div key={p.num} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', padding: '2rem' }}>
                  <p style={{ color: 'var(--text)', fontSize: '1.75rem', fontWeight: 900, marginBottom: '.75rem' }}>{p.num}</p>
                  <p className="font-bold text-sm mb-2" style={{ color: 'var(--text)' }}>{p.title}</p>
                  <p style={{ color: 'var(--text-2)', fontSize: '.8rem', lineHeight: 1.6 }}>{p.desc}</p>
                </div>
              ))}
            </div>
          </div>
