...
function SetupCard() {
  return (
    <div style={{
      background: 'var(--surface-2)', border: '1px dashed var(--border)', borderRadius: '.5rem',
      padding: '3rem 2rem', textAlign: 'center', maxWidth: 520, margin: '0 auto',
    }}>
      <div style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'center' }}>
        <svg viewBox="0 0 24 24" fill="none" stroke="var(--border)" strokeWidth="1.25" style={{ width: 48, height: 48 }}>
          <rect x="2" y="2" width="20" height="20" rx="5" ry="5"/>
          <circle cx="12" cy="12" r="4"/>
          <line x1="17.5" y1="6.5" x2="17.51" y2="6.5" strokeWidth="2.5" strokeLinecap="round"/>
        </svg>
      </div>
      <p style={{ color: 'var(--text-2)', fontSize: '.7rem', fontWeight: 900, letterSpacing: '.25em', textTransform: 'uppercase', marginBottom: '.75rem' }}>
        Instagram feed not yet connected
      </p>
      <p style={{ color: 'var(--text-3)', fontSize: '.82rem', lineHeight: 1.7, marginBottom: '2rem' }}>
        Sign up free at{' '}
        <a href="https://behold.so" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text)' }}>behold.so</a>
        , connect <strong style={{ color: 'var(--text-2)' }}>@axistrainingsystems</strong>, paste
        the Feed ID into <code style={{ color: 'var(--text-2)', background: 'var(--surface)', padding: '.1rem .4rem', borderRadius: '.2rem' }}>InstagramFeed.tsx</code>.
      </p>
      <a
...
export default function InstagramFeed() {
  return (
    <section
      id="instagram"
      style={{ background: 'var(--bg)', padding: '6rem 0', borderTop: '1px solid var(--surface)' }}
    >
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 2rem' }}>
        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <p style={{ color: 'var(--text)', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '.75rem' }}>
            Follow Along
          </p>
          <h2 style={{ color: 'var(--text)', fontWeight: 900, fontSize: 'clamp(2rem, 5vw, 3rem)', textTransform: 'uppercase', letterSpacing: '-.02em', marginBottom: '1rem' }}>
            @axistrainingsystems
          </h2>
        </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '3rem' }}>
          <a
            ...
            style={{
              ...
              background: 'transparent', border: '1.5px solid #c8102e', color: 'var(--text)',
