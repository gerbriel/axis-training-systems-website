...
function SetupCard() {
  return (
    <div style={{
      background: '#1a1a1a', border: '1px dashed #222222', borderRadius: '.5rem',
      padding: '3rem 2rem', textAlign: 'center', maxWidth: 520, margin: '0 auto',
    }}>
      <div style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'center' }}>
        <svg viewBox="0 0 24 24" fill="none" stroke="#222222" strokeWidth="1.25" style={{ width: 48, height: 48 }}>
          <rect x="2" y="2" width="20" height="20" rx="5" ry="5"/>
          <circle cx="12" cy="12" r="4"/>
          <line x1="17.5" y1="6.5" x2="17.51" y2="6.5" strokeWidth="2.5" strokeLinecap="round"/>
        </svg>
      </div>
      <p style={{ color: '#c7c7c7', fontSize: '.7rem', fontWeight: 900, letterSpacing: '.25em', textTransform: 'uppercase', marginBottom: '.75rem' }}>
        Instagram feed not yet connected
      </p>
      <p style={{ color: '#888888', fontSize: '.82rem', lineHeight: 1.7, marginBottom: '2rem' }}>
        Sign up free at{' '}
        <a href="https://behold.so" target="_blank" rel="noopener noreferrer" style={{ color: '#fff' }}>behold.so</a>
        , connect <strong style={{ color: '#c7c7c7' }}>@axistrainingsystems</strong>, paste
        the Feed ID into <code style={{ color: '#c7c7c7', background: '#0d0d0d', padding: '.1rem .4rem', borderRadius: '.2rem' }}>InstagramFeed.tsx</code>.
      </p>
      <a
        href="https://behold.so" target="_blank" rel="noopener noreferrer"
        style={{ display: 'inline-block', background: 'transparent', border: '1px solid #222222', color: '#c7c7c7', fontWeight: 700, fontSize: '.7rem', letterSpacing: '.15em', textTransform: 'uppercase', padding: '.75rem 1.75rem', borderRadius: '.25rem', textDecoration: 'none', transition: 'border-color .15s, color .15s' }}
...
export default function InstagramFeed() {
  return (
    <section
      id="instagram"
      style={{ background: '#060606', padding: '6rem 0', borderTop: '1px solid #0d0d0d' }}
    >
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 2rem' }}>
        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <p style={{ color: '#fff', fontSize: '.65rem', fontWeight: 900, letterSpacing: '.35em', textTransform: 'uppercase', marginBottom: '.75rem' }}>
            Follow Along
          </p>
